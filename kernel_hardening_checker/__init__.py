...
def print_checklist(mode: StrOrNone, checklist: list[ChecklistObjType], with_results: bool) -> None:
    ...
    if with_results:
        ...
        print(f'[+] Config check: \'OK\' - {ok_count}{ok_suppressed} / \'FAIL\' - {fail_count}{fail_suppressed}')
...
def parse_sysctl_file(mode: StrOrNone, parsed_options: dict[str, str], fname: str) -> None:
    ...
    if 'kernel.printk' not in parsed_options and mode != 'json':
        print(f'[!] WARNING: ancient sysctl options are not found in {fname}, check output of "sudo sysctl -a"')

    # let's check the presence of a sysctl option available for root
    if 'kernel.cad_pid' not in parsed_options and mode != 'json':
        print(f'[!] WARNING: root-only sysctl options not found in {fname}, check output of "sudo sysctl -a"')
    ...
    if 'permission denied' in parsed_options.values() and mode != 'json':
        print('[!] WARNING: got "permission denied" for some of sysctl options, '
              'check the output of "sudo sysctl -a" manually')
...
def main() -> None:
    ...
    parser.add_argument('-l', '--cmdline',
                        help='check the security hardening options in a kernel command line file (e.g. /proc/cmdline)')
    parser.add_argument('-s', '--sysctl',
                        help='check the security hardening options in a sysctl output file '
